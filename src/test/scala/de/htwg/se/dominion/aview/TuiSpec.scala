...
class TuiSpec extends WordSpec with Matchers {
  ...
  "A Dominion Tui" should {
    ...
    "do nothing on input 'q'" in {
      ...
      tui.stopProcessingInput should be (false)
    }
    ...
    "end the game on input 'e'" in {
      tui.processInputLine("e") should not be (controller.state.equals("turn"))
    }
    "undo on input 'u'" in {
      tui.processInputLine("u")
    }
    "redo on input 'r'" in {
      tui.processInputLine("r") should not be (controller.state.equals("end"))
    }
